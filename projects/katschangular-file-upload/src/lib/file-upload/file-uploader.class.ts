...
export class FileUploader {
  ...
  public uploadItems(values: FileItem[]): void {
    values.forEach(element => {
      const index = this.getIndexOfItem(element);
      const item = this.queue[ index ];
      item._prepareToUploading();
    });

    const transport = this.options.isHTML5 ? '_xhrTransportAll' : '_iframeTransportAll';
  ...
  protected _xhrTransportAll(items: FileItem[]): any {
    const that = this;
    const firstItem = items[0];
    const xhr = firstItem._xhr = new XMLHttpRequest();
    let sendable: any;
    ...
    if (firstItem.headers.length) {
      for (const header of firstItem.headers) {
        xhr.setRequestHeader(header.name, header.value);
      }
    }
    if (this.authToken) {
